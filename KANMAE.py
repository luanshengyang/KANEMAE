...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        # TRMs
        self.TRMs = nn.ModuleList(
            [TransformerBlock(decoder_d_model*2, attn_heads, d_ffn, enable_res_parameter, dropout) for i in range(3)])  # layers=8
...
class OurMAE(nn.Module):
    def __init__(self, args):
        ...
        seq_len = int(128/(args.wave_length)) # 128/wl args.wave_length
    ...
    def forward(self, x):
        x_I = x[:, :, 0]
        x_Q = x[:, :, 1]
        x_I_exp = x_I.unsqueeze(dim=2)  # batchsize,128,1
        x_Q_exp = x_Q.unsqueeze(dim=2)  # batchsize,128,1
        ...
        x_KAN_1 = self.predict_head_1(x_En_share_output_IQ_mean)
        x_KAN_2 = self.predict_head_2(x_KAN_1)
        x_KAN_3 = self.predict_head_3(x_KAN_2)
        return x_KAN_3
